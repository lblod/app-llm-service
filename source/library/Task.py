...
class Task:
    def __init__(self, uuid: str, queue: Queue, status: TaskStatus = TaskStatus.PENDING, type: str = "-embedding", retry_count: int = 0, log: str = "", **kwargs):
        self.uuid = uuid
        self.queue = queue
        self.status = status
        self.type = type
  ...
  # The following methods update the status, retry count, and log of a task in the task queue.
    ...
    @staticmethod
    def from_dict(json_dict, queue: Queue):

        # Extract known parameters
        known_params = {
            'queue': queue,
            'uuid': json_dict['uuid'],
            'status': TaskStatus(json_dict['status']),
        ...
        # Create the Task instance with both known and additional parameters
        return Task(**known_params, **additional_params)